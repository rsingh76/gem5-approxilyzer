...
# from gen_exec_trace
# $GEM5_DIR/build/X86/gem5.opt -d $CKPT_DIR \
...
$GEM5_DIR/build/X86/gem5.opt --outdir=$TMP_DIR \
    --debug-flags=ExecEnable,ExecUser,ExecEffAddr,ExecTicks,ExecMicro,IntRegs,FloatRegs,MiscRegs \
    --debug-file=${app_name}_dump_micro.gz $GEM5_DIR/configs/example/fs_fi.py --fi=${fi_args} \
    --disk-image=$DISK_DIR/$disk_image \
    --kernel=$kernel -r $ckpt &> $TMP_DIR/temp_${id}.txt
