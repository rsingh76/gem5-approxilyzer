...
golden_output=$5
num_procs=$6
inj_list=$APPROXGEM5/workloads/$isa/apps/$app/$1
...
if [ "$isa" == "x86" ]; then
    ...
    run_script=$APPROXGEM5/gem5/scripts/masking/injections.sh
elif [ "$isa" == "sparc" ]; then
    run_script=$APPROXGEM5/gem5/scripts/masking/run_injection_sparc.sh
fi
